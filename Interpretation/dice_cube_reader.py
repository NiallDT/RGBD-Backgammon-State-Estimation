...
from backgammon_types import CubeObservation, DiceCubeObservation, DiceObservation, NormalisedBoard
...
class DiceCubeReader:
    ...
    Dice reading is done with contour filtering + pip counting.
    Cube reading uses contour filtering and optional OCR if pytesseract is present.
    """
    ...
    def read(self, board: NormalisedBoard) -> DiceCubeObservation:
        overlay = board.rgb_bgr.copy()
        boxes = self._find_square_candidates(board.rgb_gray)
        ...
        for x1, y1, x2, y2 in boxes:
            ...
            if self.config.cube_min_area_px <= area <= self.config.cube_max_area_px and cube is None:
                ...
                cv2.putText(
                    overlay,
                    f"cube:{cube_value if cube_value is not None else '?'}",
                    (x1, y1 - 4),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.45,
                    (255, 100, 0),
